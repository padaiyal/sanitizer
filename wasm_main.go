...
func resetPageAfterAlert(alertText string) {
	jsGlobal.Call("alert", alertText)
	uploadButton := document.Call("getElementById", "upload_button")
	jsGlobal.Call("setSanitizedFilesCount", "")
	uploadButton.Set("value", "")
	jsGlobal.Call("clearOutputs")
}
...
func sanitizeCallbackFromJS(_ js.Value, _ []js.Value) any {
	...
	*/
	...
	if filesCount <= 0 {
		...
		resetPageAfterAlert("Cannot sanitize more than " + strconv.Itoa(config.MaximumInputFilesThroughWebsite) + " files at a time.\nSelect a lesser number of files.")
	} else {
		jsGlobal.Call("clearOutputs")
		jsGlobal.Call("showDisplayPanel")
		jsGlobal.Call("showSpinner")
		...
		for index := 0; index < filesCount; index++ {
			...
			if fileSizeInMB > config.MaximumInputFileSizeThroughWebsiteInMB {
				filePath := file.Get("name").String()
				resetPageAfterAlert("Size of file " + filePath + " (" + strconv.Itoa(fileSizeInMB) + " MB) exceeds maximum supported file size of " + strconv.Itoa(config.MaximumInputFileSizeThroughWebsiteInMB) + " MB.")
				return nil
			}
			files[index] = file
		}
