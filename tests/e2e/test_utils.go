...
func SetUp() {
	...
	log.Println("SETTING UP HTML SERVER")
...
func verifyFileSanitization(t *testing.T, fileDivElement selenium.WebElement, sanitizedFileInfos map[string]SanitizedFile) error {
...
func TestingValidE2E(t *testing.T, webDriver selenium.WebDriver) error {
	...
	for _, fileDivElement := range values {
		err = verifyFileSanitization(t, fileDivElement, sanitizedFileInfos)
		if err != nil {
			return err
		}
	}
