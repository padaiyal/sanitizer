...
var MaxWaitTimeout = 60 * time.Second
...
func GetDriver(driverType string) (*selenium.Service, selenium.WebDriver) {
	...
	if driverType == FIREFOX {
		...
		firefoxBinaryLocation := filepath.Join(currentPath, "resources/browsers", osType, "firefox.exe")
		if osType == "linux" {
			firefoxBinaryLocation = "/usr/bin/firefox"
		} else if osType == "darwin" {
			firefoxBinaryLocation = "/Applications/Firefox.app/Contents/MacOS/firefox"
		}
		...
		caps.AddFirefox(firefox.Capabilities{Prefs: prefs, Args: args, Binary: firefoxBinaryLocation})
